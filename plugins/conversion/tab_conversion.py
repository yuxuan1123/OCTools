"""
OCTools/plugins/conversion/tab_conversion.py
───────────────────────────────────────────────
转换页（板块一：文件转换）—— 单文件 / 多文件（逐个转换）

界面：输入（源文件 / 源文件夹）→ 目标格式（二级选择器）→ 输出 →
预设区（md→docx / 图片→docx / TTS / STT）→ 开始转换 → 实时日志。

由 ui/tabs/conversion/ 子包细分化而来（卡片在 card_*.py，动作在 actions.py，
目标格式/预设联动在 target_ctl.py，语音引擎在 engine_ctl.py，转换执行在 runner.py）。
"""
...
from plugins.conversion import (
    card_input, card_target, card_output, button_convert, card_log,
)
from plugins.conversion import actions, target_ctl, engine_ctl, runner
from plugins.conversion.registry import build_format_list
...
class TabConversion(QWidget):
    """转换页面（仅支持「转换」模式，不支持拼接）"""

    def __init__(self, parent=None):
        super().__init__(parent)
        self.mode = "转换"  # 固定为转换模式
        ...
        self.pdf_docx_config = presets.load_last_pdf_docx_config() or PdfDocxConfig()
        ...
        self.output_folder_browse_btn = None
        self.docx_card = None
        ...
        self._pdf_row_w = None
        self._pdf_summary = None
    ...
    def _build_ui(self):
        ...
        # ── 页头 ──
        lay.addWidget(build_page_header(body, "page_convert_title", "page_convert_sub"))

        # ── 输入卡 → 目标格式卡 → 输出卡 → 开始转换 → 日志卡 ──
        ...
        lay.addWidget(button_convert.build_convert_button(body, page=self))
    ...
    def _browse_output_folder(self):
        actions.browse_output_folder(self)
    ...
    def _refresh_pdf_docx_summary(self):
        target_ctl.refresh_pdf_docx_summary(self)
    ...
    def _open_pdf_docx_options(self):
        """打开 PDF→DOCX 转换方式弹窗"""
        actions.open_pdf_docx_options(self)

    def _pdf_docx_options_closed(self):
        actions.pdf_docx_options_closed(self)
    ...
    def _run_conversion(self):
        runner.run_conversion(self)


if __name__ == "__main__":
    import sys
    from PySide6.QtWidgets import QApplication
    from ui.theme import APP_STYLESHEET  # 确保这行存在

    app = QApplication(sys.argv)
    app.setStyleSheet(APP_STYLESHEET)     # ← 加上这一行

    window = TabConversion()
    window.show()
    sys.exit(app.exec())

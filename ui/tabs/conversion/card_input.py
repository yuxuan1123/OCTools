"""
OCTools/ui/tabs/conversion/card_input.py
────────────────────────────────────────
输入卡片：源文件 / 源文件夹（二选一）+ 源文件格式选择器（仅源文件夹时显示）。

对应原 tab_conversion.py 中「卡片 1：输入」整段。
"""
...
def build_input_card(parent, **ctx):
    ...
    ctx 需含 page（TabConversion），用于登记 input_entry / folder_entry /
    ...
    page.input_entry.setPlaceholderText("选择源文件（单文件转换）…")
    ...
    page.folder_entry.setPlaceholderText("选择源文件夹（多文件逐个转换）…")
    ...
    hint1 = QLabel("（源文件与源文件夹二选一；多文件支持逐个转换，输出到「输出文件夹」）", card)

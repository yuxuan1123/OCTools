...
from config.pdf_docx_config import MODE_LABELS
...
def select_format(page, fmt):
    ...
    # ── 单文件模式：维持原有输出路径联动逻辑 ──
    out = page.output_folder_entry.text().strip()
    ...
    if src and src_fmt in ("pdf", "docx") and fmt in ("jpg", "jpeg", "png"):
        ...
        if out and os.path.dirname(out):
            ...
            new_path = base + "_图片"
        page.output_folder_entry.setText(new_path)
        return

    # pptx-img 实际输出扩展名为 .pptx，自动添加 _稳定版 后缀
    ...
    if src:
        ...
        if not out:
            if fmt == "pptx-img":
                page.output_folder_entry.setText(f"{base}_稳定版.{actual_ext}")
            else:
                page.output_folder_entry.setText(f"{base}.{actual_ext}")
        else:
            out_dir = os.path.dirname(out)
            out_name = os.path.basename(out)
            out_base = os.path.splitext(out_name)[0]
            if fmt == "pptx-img" and not out_base.endswith("_稳定版"):
                out_base = f"{out_base}_稳定版"
...
def update_preset_area(page, src_fmt, dst_fmt):
    ...
    show_pdf = src_fmt == "pdf" and dst_fmt == "docx"
    ...
    page._pdf_row_w.setVisible(show_pdf)
    page._preset_none_label.setVisible(
        not (show_preset or show_img or show_tts or show_stt or show_pdf))
    if show_stt:
...
def refresh_pdf_docx_summary(page):
    """刷新「PDF→DOCX」行的当前转换方式摘要"""
    if getattr(page, "_pdf_summary", None) is None:
        return
    method = getattr(page.pdf_docx_config, "method", "libreoffice")
    page._pdf_summary.setText(MODE_LABELS.get(method, method))

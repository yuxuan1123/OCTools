...
class SetScreenRegion(QDialog):
    """截图框（OCR 识别区域）选项窗口"""

    def __init__(self, config: ScreenRegionConfig, parent=None):
        super().__init__(parent)
        self.setWindowTitle("截图框设置（OCR 识别区域）")
        ...
        # ── 命名预设管理 ──
        preset_box = QWidget(self)
        pb_lay = QVBoxLayout(preset_box)
        pb_lay.setContentsMargins(0, 6, 0, 0)
        pb_lay.setSpacing(C.size("widget_row_spacing"))
        preset_title = QLabel("区域预设（保存多套截图框配置，方便切换）", preset_box)
        preset_title.setObjectName("cardTitle")
        pb_lay.addWidget(preset_title)
        self._build_preset_row(preset_box, pb_lay)

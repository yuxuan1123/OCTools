...
class SetImageDocx(OptionsDialogBase):
    ...
    def __init__(self, config, parent=None):
        ...
        c_lay.addStretch(1)
        form.addRow("表格边框颜色:", color_row)

        hint = QLabel("提示：0 表示自动适配；勾选保持宽高比时不会拉伸变形。", body)
        hint.setObjectName("hint")
        hint.setWordWrap(True)
        form.addRow("", hint)

        # ── 按钮行 ──
        form.addRow("", self.build_buttons(

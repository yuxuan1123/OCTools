...
class SetTranslator(OptionsDialogBase):
    ...
    def __init__(self, config: TranslatorConfig, parent=None):
        ...
        self.resize(min(640, sgeo.width() - 120), min(600, sgeo.height() - 140))
        self.setMinimumSize(min(460, sgeo.width() - 60), min(400, sgeo.height() - 100))
        ...
        # ── 悬浮窗字号 + 显示模式（屏幕翻译显示，通用）──
        fs_row = QWidget(body)
        fs_lay = QHBoxLayout(fs_row)
        fs_lay.setContentsMargins(0, 0, 0, 0)
        fs_lay.setSpacing(C.size("form_row_spacing"))
        fs_lab = QLabel("屏幕翻译悬浮窗字号:", fs_row)
        fs_lab.setObjectName("fieldLabel")
        fs_lay.addWidget(fs_lab)
        self._overlay_font = QSpinBox(fs_row)
        ...
        fs_lay.addWidget(self._overlay_font)
        fs_lay.addSpacing(16)
        mode_lab = QLabel("显示模式:", fs_row)
        mode_lab.setObjectName("fieldLabel")
        fs_lay.addWidget(mode_lab)
        self._overlay_mode = QComboBox(fs_row)
        ...
        fs_lay.addWidget(self._overlay_mode)
        fs_lay.addSpacing(16)
        bg_lab = QLabel("背景:", fs_row)
        bg_lab.setObjectName("fieldLabel")
        fs_lay.addWidget(bg_lab)
        self._bg_combo = QComboBox(fs_row)
        self._bg_items = [
            ("白色（默认）", "#FFFFFF"),
            ("透明", "transparent"),
            ("深蓝黑（半透明）", "#1F2937"),
        ...
        fs_lay.addWidget(self._bg_combo)
        fs_hint = QLabel("（背景随内容自动扩宽扩高）", fs_row)
        fs_hint.setObjectName("hint")
        fs_lay.addWidget(fs_hint, 1)
        b_lay.addWidget(fs_row)
        ...
        hy_form.addRow("", hint_label(
            f"默认模型：{HY_DEFAULT_MODEL_PATH}\n"
            "Hy-MT2-1.8B 为本地 GGUF 大模型，中英双向，质量优于 Opus-MT"))
        self._stack.addWidget(fr_hy)

        # ── Opus-MT ──
        fr_opus = QWidget()
        opus_form = make_form(fr_opus)
